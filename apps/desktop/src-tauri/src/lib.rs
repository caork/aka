...
async fn client_integrations_status(
    app: tauri::AppHandle,
) -> Result<aka_server::client_integrations::ClientIntegrationsOut, String> {
    Ok(
        aka_server::client_integrations::build_client_integrations_status(
            client_integrations_dir(&app),
            None,
        ),
    )
}
...
async fn install_client_integration(
    app: tauri::AppHandle,
    request: aka_server::client_integrations::ClientIntegrationInstallRequest,
) -> Result<aka_server::client_integrations::ClientIntegrationsOut, String> {
    let resource_dir = client_integrations_dir(&app);
    tauri::async_runtime::spawn_blocking(move || {
        aka_server::client_integrations::install_client_integration(resource_dir, request)
    })
    .await
    .map_err(|e| {
