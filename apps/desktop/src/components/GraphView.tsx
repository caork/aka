  ...
  useEffect(() => {
    ...
    const genTimer = window.setTimeout(() => {
      ...
      const order = Array.from(
        ...
        (_, c) => c,
      ).sort((a, b) => data.clusterMeta[b].weight - data.clusterMeta[a].weight);
      const beacons: number[] = [];
      for (const c of order.slice(0, BEACON_COUNT)) {
        /* first node of each cluster is its hub by construction */
        let best = -1;
        let bestDeg = -1;
        grid.forEachInRect(
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      const dx = p.x - lastX;
      const dy = p.y - lastY;
      if (Math.abs(p.x - velocitySamples[0].x) > 3 || Math.abs(p.y - velocitySamples[0].y) > 3) {
        rig.moved = true;
      }
      camera.panBy(dx, dy);
      lastX = p.x;
    ...
    const frame = (t: number) => {
      ...
      if (rig.mouseDirty && !rig.dragging && rig.grid && rig.data) {
        ...
        if (i !== rig.hoverIndex) {
          ...
          setHover((h) =>
            h ? { ...h, x: rig.mouseX, y: rig.mouseY } : h,
          );
        }
      }
      ...
      <AnimatePresence>
        {!stats.ready && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            transition={spring}
            className="glass-panel absolute left-1/2 top-1/2 flex -translate-x-1/2 -translate-y-1/2 items-center gap-3 px-6 py-4"
          >
            <span
            ...
            <span className="text-[13px] font-medium text-ink-2">
              Generating 500k-node demo graph…
            </span>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {hover && (
          <motion.div
            ...
            style={{
              left: Math.min(hover.x + 14, Math.max(0, (containerRef.current?.clientWidth ?? 600) - 240)),
              top: hover.y + 14,
              maxWidth: 260,
            }}
          >
      ...
      >
        <span className="flex items-center gap-1.5">
          <span
            ...
            style={{
              background: stats.fps >= 50 || !stats.ready ? "#34c759" : "#f6a623",
            }}
          />
