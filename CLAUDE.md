...
感知所有代码的知识引擎（名字源自 Akasha records）：AKA engine native C embedded/direct facts → Rust 索引（tantivy BM25 + SQLite/CSR 图）→ MCP / HTTP / 液态玻璃桌面端 + 插件包。私有库 `caork/aka`。

**工作路径**：本仓库 `~/Documents/github/aka`；AKA engine 源码维护在 `engine/aka-engine-src/`，由 `scripts/sync-engine.sh` 构建 embedded runtime（macOS/Linux static lib、Windows `aka_engine.dll`）；运行时数据在 `~/.aka/`（registry.json + repos/<slug-hash>/{graph.db,search}/ + checkouts/）。

## 硬约束（必须遵守）

- **License**：AKA engine 含 MIT 派生代码；aka 按 MIT 口径接入与分发，保留必要来源说明。
- **AKA engine 是第一方组件，不追求旧上游运行时兼容**：解析能力改动直接在 `engine/aka-engine-src/` 的 C 源码 checkout 中完成并提交。`scripts/sync-engine.sh` 默认只构建当前 checkout，不重置本地分支。只有月度/显式上游评估时使用 `scripts/sync-engine.sh --refresh-upstream` 抓取 aka engine fork 和 upstream，然后手工 merge/rebase/cherry-pick 选择性吸收有价值 feature；脚本不得自动 reset/clean 维护分支。旧 `engine/aka-engine` / `aka-engine.exe` binary 不再作为 runtime fallback；`engine/ENGINE_SHA` 锚定当前维护的 engine commit；engine commit 变化后运行 `scripts/pin-engine-ref.sh` 同步 Docker/release 的 `AKA_ENGINE_REF`。
- **Facts 合同** `docs/contracts/artifacts.md` 是 engine/Rust 的唯一接口：字段只增不改不删；破坏性变更必须 `FACTS_VERSION` +1 并双侧同步，Rust 侧永不 import engine 内部模块。旧 engine SQLite artifact adapter、facts sidecar NDJSON、bundled engine binary 不保留为 fallback/debug。
...
- **产品形态只有桌面版 + 插件包**（用户拍板）：对外发布、文档、汇报、阶段状态和发包说明不要再说 aka CLI / CLI 版 / 裸 CLI，也不要写"aka-cli 自身编译/发包"这类口径。即使 cargo/CI 日志里出现 `aka-cli`，也只能称为"内部 runtime crate 编译/验证"。`apps/cli`、`aka-cli` crate、`AKA mcp/analyze/serve` 子命令只视为桌面包/插件宿主/headless Docker 的内部宿主与源码调试入口；用户正常使用路径是启动 AKA 桌面端和安装 Claude Code / OpenCode / Codex 插件/配置包。
...
- **Windows 快速迭代验证优先本地打包，不默认等 GitHub CI**：日常验证 GUI/Rust/Tauri 改动时，先在本机用 `cargo-xwin` 交叉编译 Windows `AKA.exe`，跳过 NSIS 安装器、updater 签名、checksums 和 Release 上传；只有准备正式发版、需要签名/updater 资产、或用户明确要求 CI release 时，才走 GitHub Actions 发布流水线。engine C 源码未变时复用上一次可信的 Windows `aka_engine.dll`；engine 变更后在 Windows/CI/可用 Windows 构建环境产出新的 `aka_engine.dll`，再作为 `AKA_ENGINE_DLL_WIN_X64` 输入本地快速包。
...
```
仓库源码 ─AKA engine embedded/direct facts API─▶ aka-facts ─aka-core 摄取─▶
...
- `crates/aka-core`：facts 合同类型、注册表（~/.aka/registry.json）、EngineRunner（embedded/direct facts 生产、进度事件、写入 graph/search）。
...
用户侧只交付 AKA 桌面端和插件包；下面命令只用于源码开发、CI、MCP stdio 宿主或 headless Docker 调试，不作为独立 CLI 产品宣传。状态汇报里要写"内部 runtime 编译/验证"，不要写"aka-cli 自身编译"。

```bash
cargo build -p aka-cli                 # 内部 runtime crate 编译验证，不是产品发包
./target/debug/aka analyze <repo>      # 内部调试：AKA engine embedded/direct facts + 索引
./target/debug/aka serve &             # 内部调试：HTTP :4111（桌面端数据源）
cd apps/desktop && npm run dev         # UI :5188（HMR；自动连 serve）
./target/debug/aka mcp                 # 插件宿主：MCP stdio（优先用桌面端 HTTP MCP）

# engine 首次初始化/构建 embedded runtime
...
# 需要可信的 Windows engine DLL；engine 未变时可复用上一轮产物
export AKA_ENGINE_DLL_WIN_X64=/absolute/path/to/aka_engine.dll

# 准备 Tauri Windows 资源（必须命名为 aka_engine.dll，不能带旧名）
rm -rf apps/desktop/src-tauri/resources/engine
mkdir -p apps/desktop/src-tauri/resources/engine
cp "$AKA_ENGINE_DLL_WIN_X64" apps/desktop/src-tauri/resources/engine/aka_engine.dll
...
cd apps/desktop
npm run tauri -- build --runner cargo-xwin --target x86_64-pc-windows-msvc --features embedded-engine --no-bundle --ci
cd ../..
...
（2026-06-10）M0–M3 完成：AKA engine native 接入 + facts 合同；Rust 五 crate 全绿（workspace 73 测试 + clippy -D warnings）；MCP/HTTP 双服务；桌面端三视图真实数据 + 仓库全生命周期（git/zip 导入、一键更新、per-repo 设置、删除）+ 节点详情/ego 下钻；WebGL 50 万节点/百万边 60fps 实测。
...
**待办**：M4 Docker 化 + Jensen 部署 + 远程模式；embedding 保持默认关闭且暂不作为当前重点；增量索引（fileHashes/parse-cache）；Tauri 正式打包（内置 AKA engine embedded runtime）；wiki/group 按需补齐；导入中 update 返回 404 的小语义瑕疵。
