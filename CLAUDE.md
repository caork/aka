...
- **Windows 快速迭代验证优先本地打包，不默认等 GitHub CI**：日常验证 GUI/Rust/Tauri 改动时，先在本机用 `cargo-xwin` 交叉编译 Windows `AKA.exe`，跳过 NSIS 安装器、updater 签名、checksums 和 Release 上传；只有准备正式发版、需要签名/updater 资产、或用户明确要求 CI release 时，才走 GitHub Actions 发布流水线。engine C 源码未变时复用上一次可信的 Windows `aka-engine.exe` 和 `aka_engine.dll`；engine 变更后先在 Windows/CI/可用 Windows 构建环境产出新的 `aka-engine.exe`，并用 MinGW/MSYS2 产出 `aka_engine.dll`，再作为 `AKA_ENGINE_BIN_WIN_X64` / `AKA_ENGINE_DLL_WIN_X64` 输入本地快速包。
...
# 需要可信的 Windows engine exe + DLL；engine 未变时可复用上一轮产物
...
# 准备 Tauri Windows 资源（必须命名为 aka-engine.exe / aka_engine.dll，不能带旧名）
