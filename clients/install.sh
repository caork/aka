...
#   - Claude Code / Codex 加 --stdio 时才改为按需启动 `AKA mcp` 作为 fallback
...
#                 (marketplace add 本仓库 + plugin install aka@aka, 含 HTTP MCP + skill)
#   codex       : 默认 `codex mcp add aka --url ...` 或追加 url 到 ~/.codex/config.toml
...
CLIENT=""
BIN=""
MCP_URL="${AKA_MCP_URL:-http://127.0.0.1:4112/mcp}"
DRY_RUN=0
USE_PLUGIN=0
USE_STDIO=0
...
if [ "$USE_STDIO" -eq 1 ] && [ "$CLIENT" != "opencode" ]; then
  ...
  warn "默认 HTTP MCP 模式不会使用 --bin 指定的路径: ${BIN}；需要 stdio fallback 时加 --stdio。"
elif [ -n "$BIN" ]; then
  warn "OpenCode 默认连接 ${MCP_URL}，不会使用 --bin 指定的路径: ${BIN}"
fi
...
install_claude_code() {
  ...
  if [ "$USE_PLUGIN" -eq 1 ]; then
    ...
    if [ "$USE_STDIO" -eq 1 ]; then
      ...
      info "提示: 想要捆绑 skill 的插件方式，用 --plugin 重跑；桌面不常开时可用 --stdio 配 fallback。"
    fi
  fi
}
...
install_codex() {
  ...
  if [ "$USE_STDIO" -eq 1 ] && command -v codex >/dev/null 2>&1; then
  ...
  elif [ "$USE_STDIO" -eq 0 ] && command -v codex >/dev/null 2>&1; then
    run codex mcp add aka --url "${MCP_URL}"
  else
    ...
    if [ "$DRY_RUN" -eq 1 ]; then
      if [ "$USE_STDIO" -eq 1 ]; then
        ...
        info "[dry-run] 将追加: [mcp_servers.aka] url=\"${MCP_URL}\""
      fi
    else
      mkdir -p "${HOME}/.codex"
      {
  ...
  if [ "$USE_STDIO" -eq 1 ]; then
    ...
    info "完成。先启动 AKA 桌面端，再验证: codex mcp list（或 Codex 里 /mcp）"
  fi
}
...
install_opencode() {
  ...
  if ! command -v jq >/dev/null 2>&1; then
    ...
    printf '{\n  "mcp": {\n    "aka": { "type": "remote", "url": "%s", "enabled": true }\n  }\n}\n' "${MCP_URL}"
    install_opencode_plugin
    install_opencode_skill
    return
  fi
  if [ "$DRY_RUN" -eq 1 ]; then
    info "[dry-run] 将向 ${cfg} 合并 mcp.aka = {type:remote, url:${MCP_URL}, enabled:true}"
    install_opencode_plugin
    install_opencode_skill
    return
  fi
  ...
  local tmp
  tmp="$(mktemp)"
  jq --arg url "${MCP_URL}" '.mcp.aka = {type: "remote", url: $url, enabled: true}' "$cfg" > "$tmp"
  mv "$tmp" "$cfg"
