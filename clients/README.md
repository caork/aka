# aka 客户端接入

把 aka 接进编码 agent 客户端：默认都连接 AKA 桌面端内置的本地 Streamable HTTP MCP（`http://127.0.0.1:4112/mcp`），让 AI 索引/查询和 GUI 图谱预览复用同一份知识库。Claude Code / Codex 仍可用 `AKA mcp` 作为 stdio fallback。设计文档见 [docs/clients.md](../docs/clients.md)。

## 一键安装

```bash
clients/install.sh --check                       # 检测 AKA MCP 与三客户端配置
clients/install.sh --client opencode             # 默认连接已运行的 AKA 桌面端，需要 jq
clients/install.sh --client claude-code          # 默认连接已运行的 AKA 桌面端
clients/install.sh --client codex                # 默认连接已运行的 AKA 桌面端
clients/install.sh --client claude-code --plugin # 插件方式（HTTP MCP + skill）
clients/install.sh --client codex --stdio --bin /path/to/AKA  # 桌面不常开时的 fallback
# 任意客户端都可加 --dry-run 预览；加 --reinstall 会备份/覆盖 aka 管理的配置项
```

Windows PowerShell：

```powershell
.\clients\install.ps1 -Check
.\clients\install.ps1 -Client claude-code -Plugin -Reinstall
.\clients\install.ps1 -Client codex -Reinstall
.\clients\install.ps1 -Client opencode -Reinstall
```

脚本幂等：已配置过会提示并跳过；重装只更新 aka 相关配置，Codex 配置会先备份。

## 三客户端速览（2026-06 核实的格式）

| 客户端 | 配置位置 | 格式要点 | 验证 |
|---|---|---|---|
| **Claude Code** | 插件（`.claude-plugin/plugin.json` + `.mcp.json`）或 `claude mcp add` | 默认 HTTP MCP 指向 `http://127.0.0.1:4112/mcp`；stdio fallback 用 `AKA mcp` | `claude mcp list` / `claude plugin list` |
| **Codex CLI** | `~/.codex/config.toml` | 默认 `[mcp_servers.aka] url = "http://127.0.0.1:4112/mcp"`；stdio fallback 用 `command` + `args` | `codex mcp list`，TUI 里 `/mcp` |
| **OpenCode** | `~/.config/opencode/opencode.json` 或项目根 `opencode.json`；插件在 `~/.config/opencode/plugins/` 或 `.opencode/plugins/` | `mcp.aka` 对象，`type: "remote"`，`url: "http://127.0.0.1:4112/mcp"`；本地 plugin 是 JS/TS module | 启动 AKA 桌面端后，在 TUI 会话里触发 `aka_list_repos` |

各目录详情：

- [claude-code/](./claude-code/) — 完整插件（MCP server + `aka-code-graph` skill），可 `claude plugin marketplace add` 本仓库或 `clients/` 发布包后 `claude plugin install aka@aka`
- [codex/](./codex/) — TOML 片段 + `codex mcp add` 用法 + 可贴进 `AGENTS.md` 的 aka 使用策略
- [opencode/](./opencode/) — JSON 片段 + OpenCode 本地 plugin + 使用策略（原生 skill 推荐，AGENTS-aka.md 备选），发布包 `aka-opencode-plugin-<ver>.zip` 即此目录

## 通用前置

1. 默认复用已经运行的 AKA 桌面端本地 MCP server：`http://127.0.0.1:4112/mcp`。
2. agent 会先调 `list_repos`：HTTP MCP 会优先读取客户端 workspace roots 并自动排队索引；如果客户端不暴露 roots，服务端会用进程工作目录兜底，后续工具传本地路径也会自动提升到 git/project root 并排队索引；`status: "indexing"` 表示自动索引正在跑，稍后重试。
3. 需要强制索引非当前本地工作区时，显式调用 `analyze` 并传仓库绝对路径、相对路径或仓库内任意子目录；这些管理工具会写 aka 本地索引/受管 checkout。
4. 如果要分析远程 GitHub/Git 仓库，让 agent 调 `import_repo`，传 `kind:"git"` 和 clone URL；需要刷新已索引仓库时调 `update_repo`。
5. 桌面不常开或客户端不支持 HTTP MCP 时，Claude Code / Codex 可加 `--stdio --bin /path/to/AKA` 使用 `AKA mcp` fallback；fallback 与 GUI 共用同一份桌面数据。
6. `rename dry_run:false` 会改工作区文件；agent 应先看 dry-run 计划并遵守客户端审批。

源码开发时仍可 `cargo build --release -p aka-cli` 做内部 runtime crate 编译验证；脚本也会自动探测 `target/{release,debug}/aka`、桌面包里的 `AKA` 和 PATH 上的 `aka`，方便本地调试。它不是用户可见的独立产品形态。

> License 提醒：解析引擎 AKA engine 为 MIT，aka 客户端接入按 MIT 口径分发。
