...
- 目标仓库不在列表里 → 用 `analyze`（参数 `repo_path` 必须是**绝对路径**，优先传当前项目根目录）确保索引；aka 会把本地仓库注册到同一份 GUI 可见知识库并后台索引，已注册仓库则排队更新。返回里的 `repo` 是后续工具要传的仓库名，`status:"indexing"` 时稍后先重试 `list_repos`。
