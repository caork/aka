---
name: aka-code-graph
description: 用 aka 代码知识图谱（17 个 MCP 工具：list_repos/query/search_code/context/find_definition/search_references/impact/detect_changes/route_map/tool_map/graphql_map/shape_check/api_impact/analyze/import_repo/update_repo/augment）高效检索和理解仓库；HTTP MCP 会优先复用 AKA 桌面端，且每次工具调用都会尝试读取客户端 workspace roots 自动排队索引；stdio fallback 也会自动发现当前工作区。当需要在大代码库里找符号定义、搜实现、评估改动影响面（blast radius）、检查当前改动、或查看 API route/GraphQL/tool 映射时使用。比逐文件 grep/read 更省 token、更准。
---

# aka 代码知识图谱使用策略

aka 把仓库解析成「符号节点 + 调用/引用/应用语义边」的图，并建了 BM25 全文索引。十七个 MCP 工具覆盖五类任务：**检索**（query/search_code/augment）、**定位**（find_definition/context/search_references）、**影响分析**（impact/detect_changes/api_impact/shape_check）、**应用映射**（route_map/graphql_map/tool_map），外加 **管理**（list_repos/analyze/import_repo/update_repo）。
...
> OpenCode 里 MCP 工具按 `<server>_<tool>` 命名：上面十七个工具显示为 `aka_list_repos`、`aka_query` 等。下文用短名。
