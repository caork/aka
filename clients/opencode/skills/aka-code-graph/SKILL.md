---
name: aka-code-graph
description: 用 aka 代码知识图谱（19 个 MCP 工具：list_repos/query/search_code/context/find_definition/search_references/impact/rename/detect_changes/route_map/tool_map/graphql_map/topic_map/shape_check/api_impact/analyze/import_repo/update_repo/augment）高效检索、理解和安全重命名仓库；HTTP MCP 会优先复用 AKA 桌面端，每次工具调用都会先读客户端 workspace roots 自动排队索引，roots 不可用时用服务端工作目录和本地路径参数兜底；stdio fallback 也会自动发现当前工作区。当需要在大代码库里找符号定义、搜实现、评估改动影响面（blast radius）、安全 rename、检查当前改动、或查看 API route/GraphQL/tool/message topic 映射时使用。比逐文件 grep/read 更省 token、更准。
---
