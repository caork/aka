...
把 aka 作为 Claude Code 插件安装：捆绑 **AKA 桌面端本地 HTTP MCP 配置**（十四工具）+ **aka-code-graph skill**（指导 agent 何时用哪个工具）。AI 的索引/查询结果会进入同一份 GUI 可见知识库。
...
Claude Code 通过这个本地 HTTP MCP 调用 aka 工具；不需要单独启动 CLI 版 `aka`。
