...
struct SynthTopic {
    ...
    name: String,
    broker: String,
    producers: Vec<SynthTopicEndpoint>,
    consumers: Vec<SynthTopicEndpoint>,
}

#[derive(Debug, Clone, Eq, PartialEq, Ord, PartialOrd)]
struct SynthTopicEndpoint {
    node_id: String,
    file_path: String,
    strategy: String,
}
...
fn synthesize_topics_from_sources(
    repo: &Path,
    nodes: &BTreeMap<String, SynthNode>,
) -> Vec<SynthTopic> {
...
fn find_call_args<'a>(text: &'a str, callee: &str) -> Vec<CallArgs<'a>> {
    let mut out = Vec::new();
    let mut offset = 0usize;
