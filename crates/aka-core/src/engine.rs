...
use route_python_prefix_synth::{
    python_route_prefixes_for_node, python_router_prefixes_by_file, PythonRoutePrefixes,
};
...
fn extract_annotated_routes(
    ...
    for node in nodes {
        let python_decorator_route = is_python_route_node(node)
            .then(|| python_decorator_route(&node.decorators))
            .flatten();
