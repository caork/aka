...
use super::{
    find_call_args, node_at_offset, nodes_by_file, pick_handler_node, read_repo_text, stable_hash,
...
pub(super) fn synthesize_transactions_from_sources(
    ...
    let by_file = transaction_nodes_by_file(nodes);
...
fn transaction_nodes_by_file(
    nodes: &BTreeMap<String, SynthNode>,
) -> BTreeMap<String, Vec<&SynthNode>> {
    nodes_by_file(nodes)
        .into_iter()
        .filter(|(file_path, file_nodes)| {
            is_transaction_source_path(file_path)
...
fn is_transaction_source_path(file_path: &str) -> bool {
    matches!(
