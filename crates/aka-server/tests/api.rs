...
use aka_server::{
    router, Backend, MockBackend, RepoInfo, RepoSettingsUpdate, SearchHit, SymbolRef,
};
use axum::body::Body;
...
async fn query_returns_hits() {
    ...
    assert_eq!(processes[0]["id"], "demo:proc:request-flow");
...
async fn search_code_returns_raw_matches_and_directory_distribution() {
    let res = app()
        .oneshot(post_json(
            "/api/search/code",
            json!({ "repo": "demo", "query": "parse_config", "context": 1 }),
        ))
        .await
        .unwrap();
    assert_eq!(res.status(), StatusCode::OK);
...
async fn management_endpoints_are_501_on_mock() {
    ...
    let cases: Vec<Request<Body>> = vec![
        ...
        Request::post("/api/repos/demo/update")
            .body(Body::empty())
            .unwrap(),
        Request::delete("/api/repos/demo")
            .body(Body::empty())
            .unwrap(),
        ...
        Request::get("/api/graph/ego?repo=demo&id=demo:fn:main")
            .body(Body::empty())
            .unwrap(),
        Request::get("/api/source?repo=demo&path=src/main.rs")
            .body(Body::empty())
            .unwrap(),
        Request::get("/api/file/symbols?repo=demo&path=src/main.rs")
            .body(Body::empty())
            .unwrap(),
        Request::get("/api/files?repo=demo")
            .body(Body::empty())
            .unwrap(),
        multipart_req(
...
async fn source_endpoint_contract_and_errors() {
    ...
    let res = managed()
        .oneshot(
            Request::get("/api/source?repo=demo")
                .body(Body::empty())
                .unwrap(),
        )
        .await
...
async fn file_symbols_contract_and_errors() {
    ...
    let res = managed()
        .oneshot(
            Request::get("/api/file/symbols?repo=demo")
                .body(Body::empty())
                .unwrap(),
        )
        .await
