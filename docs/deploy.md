...
4. `docker save` 的镜像 tar 与 x86_64 linux 二进制挂到同名 GitHub Release——
   Jensen 等目标机离线 `docker load -i aka-<版本>-linux-amd64.docker.tar.gz` 即用。
...
macOS 本机二进制由 `scripts/package-release.sh` 产出（不含 Docker）。
