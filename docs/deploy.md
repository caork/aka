...
桌面包必须内置 native `aka-engine`。`scripts/package-release.sh` 会在准备 Tauri 资源和生成 macOS
`.app.zip` 时校验 `engine/aka-engine` / `engine/aka-engine.exe`，避免把旧的 JS/node engine 目录打进包里却没有
native binary。Windows portable 的用户形态固定为单文件 `AKA.exe`，不把 `engine\aka-engine.exe` 或
`aka_engine.dll` 作为外置文件发布；`AKA.exe` 内置 `aka_engine.dll` 作为 direct-facts 默认路径，并内置
`aka-engine.exe` 作为 fallback/debug。发布验证以 Windows 侧启动 `AKA.exe` 后通过 MCP 完成索引构图和查询为准。
