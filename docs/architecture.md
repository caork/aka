...
立项 2026-06-10。当前方向：使用第一方 AKA engine 原生 C 引擎负责多语言 tree-sitter/LSP 解析，并逐步从“engine SQLite -> NDJSON artifacts”迁移到“engine / SCIP / stack-graphs 直接产出 `aka-facts`”。Rust 侧保留存储、搜索、服务和桌面体验，并把 facts 直接写入 graph/search。

## 总体策略

Rust workspace 承担存储、搜索、服务、UI。新的热路径合同是 `aka-facts`（见 [contracts/artifacts.md](contracts/artifacts.md)）：engine、SCIP importer、tree-sitter stack-graphs adapter、业务 enrichment job 都产出可重放 facts，graph/search writer 直接消费 `FactSource`。旧 AKA engine binary + SQLite + artifact 目录只作为兼容 fallback 和调试导出保留。

不在 Rust 侧手写多语言 parser，也不引入 WASM tree-sitter worker 池。解析层优先使用 AKA engine embedded/direct fact API；语言生态已有能力通过 SCIP 或 stack-graphs 接入；LSP 只作为 unsupported language 或 live fallback。业务语义补全必须变成有预算的 enrichment facts，不能阻塞 baseline index ready。
...
- 前端直接 invoke Rust 命令；索引进度走 Tauri event（facts discover/parse/fuse、enrichment、graph/search 分阶段展示）。legacy `export-artifacts` 阶段只在兼容 adapter 中出现。
...
- M0 地基：AKA engine native + direct facts contract / legacy artifact fallback + demo/golden 冒烟
