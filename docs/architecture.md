...
外部 analyzer adapter 可以通过 `ossAnalyzerFactsPath` 交付 `aka-facts` JSON/JSONL bundle；这是显式配置的 OSS analyzer 结果导入入口，不是旧 sidecar/fallback/debug transport。adapter 可在 bundle 顶层声明 `analyzerId`/`toolVersion`，由 runtime 统一 stamp provenance 后校验。runtime 只校验、合并和索引这些 facts，不启动语言服务、不扫描源码、不做业务语义推断。SCIP 先用 `scripts/smoke-oss-analyzer-scip.sh` 在 Apache Dubbo 这类 50 万行以上仓库跑大仓 smoke，确认 baseline ready、optional provider outcome 可见、查询非空后再扩大到其它 analyzer。
