...
外部 analyzer adapter 可以通过 `ossAnalyzerFactsPath` 交付 `aka-facts` JSON/JSONL bundle；这是显式配置的 OSS analyzer 结果导入入口，不是旧 sidecar/fallback/debug transport。adapter 可在 bundle 顶层声明 `analyzerId`/`toolVersion`，由 runtime 统一 stamp provenance 后校验。runtime 只校验、合并和索引这些 facts，不启动语言服务、不扫描源码、不做业务语义推断。SCIP 用 `scripts/smoke-oss-analyzer-scip.sh` 在 Apache Dubbo 这类大仓跑 importer smoke；stack-graphs Python 用 `scripts/oss-analyzer-stack-graphs-python.mjs` + `scripts/smoke-oss-analyzer-stack-graphs-python.sh` 在 CPython 这类 50 万行以上 Python 仓库跑 definition facts smoke；Pyright 用 `scripts/oss-analyzer-pyright-lsp.mjs` + `scripts/smoke-oss-analyzer-pyright.sh` 在 CPython 这类 50 万行以上仓库跑 LSP facts smoke；gopls 用 `scripts/oss-analyzer-gopls-lsp.mjs` + `scripts/smoke-oss-analyzer-gopls.sh` 在 Kubernetes 这类 50 万行以上 Go 仓库跑 LSP facts smoke；TypeScript 用 `scripts/oss-analyzer-typescript-lsp.mjs` + `scripts/smoke-oss-analyzer-typescript.sh` 在 Microsoft TypeScript 这类 50 万行以上 TS/JS 仓库跑 LSP facts smoke；rust-analyzer 用 `scripts/oss-analyzer-rust-analyzer-lsp.mjs` + `scripts/smoke-oss-analyzer-rust-analyzer.sh` 在 rust-lang/rust 这类 50 万行以上 Rust 仓库跑 LSP facts smoke；JDTLS 用 `scripts/oss-analyzer-jdtls-lsp.mjs` + `scripts/smoke-oss-analyzer-jdtls.sh` 在 Apache Cassandra 这类 50 万行以上 Java 仓库跑 LSP facts smoke。这些路径都要求 baseline ready、optional provider outcome 可见、查询非空，再扩大到其它 analyzer。
...
- M3 Tauri 桌面 MVP
- M4 headless + Docker + 远程模式；LSP/SCIP/stack-graphs provider 按热门开源实现逐个接入，先过 allowlist + provenance 覆盖 + 大仓基准，再决定是否默认可用
