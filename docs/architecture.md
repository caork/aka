...
立项 2026-06-10。当前方向：把慢速 TypeScript 解析链路切换为 `codebase-memory-mcp` 原生 C 引擎。CBM 负责多语言 tree-sitter/LSP 解析并写入 SQLite；aka 保留 Rust 存储、搜索、服务和桌面体验，通过 SQLite->NDJSON adapter 维持稳定工件合同。

## 总体策略

Rust workspace 承担存储、搜索、服务、UI；CBM 作为 native C binary 进程运行，解析结果先落到 CBM SQLite，再由 `aka-core` adapter 导出为 NDJSON 工件。两者之间用工件合同隔离（见 [contracts/artifacts.md](contracts/artifacts.md)）。

不在 aka 内手写多语言 parser，也不引入 WASM tree-sitter worker 池。解析层使用 CBM 的 native C 实现，aka 只承担 adapter 和后续索引，避免 JS 解析运行时进入 indexing 热路径。
...
- 图存储：SQLite（rusqlite）持久 + 启动构建内存 CSR 邻接；callers/callees/impact 走内存遍历。aka 服务面不暴露 CBM 的 Cypher 查询语义，仍走 Rust 邻接 API。
...
- GitNexus-like 能力：query/context/impact 会消费合成 Community/Process/Command/Transaction，新增 detect_changes/route_map/tool_map/graphql_map/shape_check/api_impact 消费 Route/GraphQL/Tool/Command/FETCHES/HANDLES_ROUTE/HANDLES_GRAPHQL/HANDLES_TOOL/HANDLES_COMMAND/HAS_TRANSACTION_BOUNDARY/ENTRY_POINT_OF/STEP_IN_PROCESS 等索引语义，覆盖流程分组、改动到流程映射、API 路由/GraphQL operation/工具入口、CLI/management command、事务边界和响应形状检查。它是面向 agent 工作流的保守兼容层，不提供完整 GitNexus 图模型、Cypher 查询或完全等价的跨语言语义。
...
- 前端直接 invoke Rust 命令；索引进度走 Tauri event（CBM stdout + adapter 阶段事件透传）。

## 部署形态（一套核心，三种交付）

| 形态 | 内容 | 场景 |
|---|---|---|
| 桌面 app | Tauri（UI + core + CBM native engine 全打包） | Mac 日常使用，本地分析 |
...
- M0 地基：CBM native engine + SQLite->NDJSON 工件层 + demo/golden 冒烟
...
- License：CBM 为 MIT，aka 按 MIT 口径打包；Rust 侧与 engine 以进程边界 + 工件合同隔离，将来如需换血只换 engine adapter。
- 上游漂移：合同字段只增不改；同步 CBM 后先通过 adapter/golden 冒烟确认。
- 跨平台打包：macOS/Linux 直接内置 `codebase-memory-mcp`，Windows 需要对应 `.exe` 资源；Tauri 与 Docker 的解析链路都不再依赖 JS runtime。
