...
- GitNexus-like 能力：query/context/impact 会消费合成 Community/Process，新增 detect_changes/route_map/tool_map/graphql_map/shape_check/api_impact 消费 Route/GraphQL/Tool/FETCHES/HANDLES_ROUTE/HANDLES_GRAPHQL/HANDLES_TOOL/ENTRY_POINT_OF/STEP_IN_PROCESS 等索引语义，覆盖流程分组、改动到流程映射、API 路由/GraphQL operation/工具入口和响应形状检查。它是面向 agent 工作流的保守兼容层，不提供完整 GitNexus 图模型、Cypher 查询或完全等价的跨语言语义。
