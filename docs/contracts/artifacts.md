...
AKA 的热路径合同是 `aka-facts`：parser / engine / SCIP importer / stack-graphs adapter 产出稳定 facts，Rust 侧直接把这些 facts 写入 graph/search。旧的 `artifact/` 目录仍保留为一个兼容和调试 transport，但不再是架构上的唯一接口。
...
```
aka-engine binary -> engine SQLite -> legacy artifact dir -> ArtifactDir(FactSource)
  -> graph/search writer
```
...
事实合同版本为 `aka_facts::FACTS_VERSION = 1`。旧 artifact 目录仍使用 `contractVersion = 0`，由 `ArtifactDir` 适配为 `FactSource`。
...
Direct facts 不依赖 `manifest.json` 作为完成标记。一个 producer 完成时必须发送 `Done { stats }` 或返回完整 `FactBatch`；调试导出才需要把 manifest 写到磁盘。
...
```json
{"nodeId":"...","kind":"ast-function","filePath":"...","startLine":0,"endLine":19,"text":"..."}
```

`kind`: `ast-function` | `ast-declaration` | `char` | producer 自定义字符串。向量 embedding 仍由 AKA 设置控制，默认关闭；chunk 只是索引候选文本。
...
4. LSP adapter：只作为 unsupported language 或 live fallback，不作为批量索引主路径。
...
旧 `aka-engine:export-artifacts:*` 阶段只允许 legacy adapter 使用。UI 应把 facts / enrichment / index 分开展示，不再把多个语义阶段折叠成一个 “artifacts” 阶段。

## Legacy artifact transport v0
...
`manifest.json` 的 `contractVersion` 仍为 `0`，并且最后写入作为 legacy 完成标记。`nodes.ndjson` / `edges.ndjson` / `chunks.ndjson` 的 JSON shape 与 `NodeFact` / `EdgeFact` / `ChunkFact` 一致，因此 `ArtifactDir` 可以实现 `FactSource`。

新功能不得依赖 engine SQLite schema 或 legacy NDJSON 文件作为必经路径。需要排障时可以显式导出 facts/artifacts 供人工检查。

## Version discipline

- facts 字段只增不改不删；破坏性变更必须 bump `FACTS_VERSION`。
- legacy artifact v0 只做兼容维护，不承载新能力。
- engine 内部 SQLite schema 不再是 aka-core 合同。schema 漂移只能影响 legacy adapter，不能影响 direct facts writer。
