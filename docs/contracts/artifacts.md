...
`aka-facts/scip-import` 只读取已经存在的 `index.scip` 并转换为 `FileFact` / `SymbolFact` / `OccurrenceFact` / `RelationFact`；它不启动语言 indexer，也不执行自研源码扫描。具体 Java/Python/TypeScript/Rust 等 indexer 的安装、执行和超时控制属于 provider 层，必须继续遵守上述 allowlist、provenance、非阻塞和大仓基准规则。

内部 runtime 的 optional enrichment merge 只在 baseline graph/search ready 后追加 facts：新节点和对应 chunks 追加到 search，边写入 graph 并依赖 provenance edge id 去重。merge 失败或 provider 失败只能产生 failed/skipped outcome 与日志；已有 graph/search 不回滚、不置 failed、不阻塞查询。
