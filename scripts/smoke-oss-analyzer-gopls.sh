#!/usr/bin/env bash
# Smoke optional gopls LSP enrichment on a real Go repository.
# AKA runtime imports the generated aka-facts bundle; it does not start gopls.
set -euo pipefail

usage() {
  cat <<'EOF'
Usage:
  scripts/smoke-oss-analyzer-gopls.sh --repo /path/to/repo [options]
...
Options:
  --facts PATH             Existing/generated aka-facts bundle path.
                           Defaults to REPO/.aka/gopls-oss-analyzer-facts.json.
  --server CMD             gopls LSP command. Default: gopls serve
  --tool-version VERSION   Override analyzer toolVersion in the generated bundle.
  --query TEXT             Search query after indexing. Default: kubelet
  --context SYMBOL         Optional symbol context query after indexing.
  --min-lines N            Required Go source line count. Default: 500000.
  --aka-home PATH          Isolated AKA_HOME. Defaults to a temp dir.
  --index-max-secs N       Global indexing budget. Default: 1800.
  --enrichment-max-secs N  Optional gopls import budget. Default: 600.
  --adapter-timeout-secs N gopls adapter generation budget. Default: 900.
  --max-files N            Optional cap for adapter smoke/debug runs.
  --concurrency N          Concurrent gopls documentSymbol requests. Default: 16.
  --allow-small-repo       Allow Go source lines below --min-lines for adapter debugging.
  --keep-aka-home          Do not delete the temporary AKA_HOME.
  --skip-generate          Reuse --facts without running gopls.
  --dry-run                Validate inputs and generated settings, then exit.

Example:
  scripts/smoke-oss-analyzer-gopls.sh --repo ~/src/kubernetes \
    --server '/tmp/aka-large-smoke/bin/gopls serve' \
    --tool-version gopls-v0.22.0 \
    --query kubelet --context NewKubeletCommand
EOF
}
...
repo="$(cd "${repo}" && pwd -P)"
facts_path="${facts_path:-${repo}/.aka/gopls-oss-analyzer-facts.json}"
...
line_count="$(
  {
    while IFS= read -r -d '' file; do
      ...
      \( -path '*/.git' -o -path '*/vendor' -o -path '*/_output' -o -path '*/target' -o -path '*/node_modules' -o -path '*/build' -o -path '*/dist' \) -prune \
...
echo "==> repo: ${repo}"
echo "==> Go source lines: ${line_count}"
echo "==> facts bundle: ${facts_path}"
echo "==> gopls server: ${server}"

if [[ "${line_count}" -lt "${min_lines}" ]]; then
  if [[ "${allow_small_repo}" -eq 0 ]]; then
    die "Go source line count ${line_count} is below requested ${min_lines}; pass --allow-small-repo only for adapter debugging"
  fi
  echo "warning: Go source line count ${line_count} is below requested ${min_lines}" >&2
fi

if [[ -z "${aka_home}" ]]; then
  aka_home="$(mktemp -d "${TMPDIR:-/tmp}/aka-gopls-smoke.XXXXXX")"
...
if [[ "${skip_generate}" -eq 0 ]]; then
  echo "==> generating aka-facts with external gopls LSP"
  adapter_args=(
    --repo "${repo}"
    --out "${facts_path}"
    --server "${server}"
  ...
  node "${repo_root}/scripts/oss-analyzer-gopls-lsp.mjs" "${adapter_args[@]}"
fi
...
log_path="${aka_home}/gopls-smoke.log"
echo "==> running baseline indexing plus optional gopls enrichment"
(
  cd "${repo_root}"
...
grep -Eq 'provider=aka-facts-file merged|optional enrichment outcome' "${log_path}" \
  || die "gopls facts enrichment outcome was not visible; see ${log_path}"

if grep -Eq 'provider=aka-facts-file failed|merge_failed|invalid_provenance' "${log_path}"; then
  die "gopls facts enrichment failed; see ${log_path}"
fi
...
echo "==> gopls OSS analyzer smoke passed"
