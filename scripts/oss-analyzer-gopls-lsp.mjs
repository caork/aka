#!/usr/bin/env node
// Convert gopls LSP document symbols into an aka-facts OSS analyzer bundle.
//
// This adapter is intentionally outside the AKA runtime. It starts the external
// open-source gopls language server, asks it for document symbols, and writes a
// facts JSON bundle that AKA can validate and import through ossAnalyzerFactsPath.
...
const DEFAULT_SERVER = "gopls serve";
...
function usage() {
  console.error(`Usage:
  scripts/oss-analyzer-gopls-lsp.mjs --repo PATH --out PATH [options]
...
Options:
  --server CMD             gopls LSP command. Default: ${DEFAULT_SERVER}
...
Example:
  scripts/oss-analyzer-gopls-lsp.mjs \\
    --repo /src/kubernetes \\
    --out /src/kubernetes/.aka/gopls-oss-analyzer-facts.json \\
  ...
  constructor(child, deadlineAt) {
    ...
    child.stderr.on("data", (chunk) => {
      for (const line of chunk.toString("utf8").split(/\r?\n/)) {
        if (line.trim()) {
          console.error(`[gopls] ${line}`);
        }
      }
    });
    child.on("exit", (code, signal) => {
      const err = new Error(`gopls language server exited code=${code} signal=${signal}`);
...
async function discoverGoFiles(root, maxFiles) {
  const files = [];
  async function walk(dir) {
    ...
    for (const entry of entries) {
      ...
      } else if (entry.isFile() && GO_EXTENSIONS.has(path.extname(entry.name))) {
...
function flattenDocumentSymbols(symbols, fileNodeId, rel, lines, options) {
  ...
  function visit(symbol, parents) {
    ...
    if (parent?.nodeId) {
      edges.push({
        id: stableId("gopls:edge:contains", `${parent.nodeId}->${nodeId}`),
        sourceId: parent.nodeId,
        targetId: nodeId,
        type: "CONTAINS",
        confidence: 1,
        reason: "gopls documentSymbol hierarchy",
        evidence: { source: "lsp", rule: "documentSymbol", filePath: rel },
      });
    } else {
      edges.push({
        id: stableId("gopls:edge:defines", `${fileNodeId}->${nodeId}`),
        sourceId: fileNodeId,
        targetId: nodeId,
        type: "DEFINES",
        confidence: 1,
        reason: "gopls documentSymbol top-level definition",
        evidence: { source: "lsp", rule: "documentSymbol", filePath: rel },
      });
    }
    if (!options.noChunks) {
...
async function main() {
  ...
  const files = await discoverGoFiles(repo, args.maxFiles);
  if (files.length === 0) {
    throw new Error(`no Go files found in ${repo}`);
  }
  console.error(`[aka-gopls] repo=${repo} files=${files.length} out=${out} server=${args.server}`);
  ...
  try {
    initResult = await rpc.request("initialize", {
      ...
      clientInfo: { name: "aka-gopls-lsp-adapter", version: "0.1" },
      rootPath: repo,
      rootUri,
    ...
    await mapWithConcurrency(files, args.concurrency, async (file, index) => {
      ...
      if (index % args.logEvery === 0) {
        console.error(
          `[aka-gopls] documentSymbol ${index + 1}/${files.length} ${rel} concurrency=${args.concurrency}`,
        );
      }
      const text = await readFile(file, "utf8");
      const uri = pathToFileURL(file).toString();
      const fileNodeId = stableId("gopls:file", rel);
      const fileNodes = [{
        id: fileNodeId,
        label: "File",
        properties: {
      ...
      try {
        ...
        console.error(`[aka-gopls] skip ${rel}: ${error.message}`);
      } finally {
    ...
    if (nodes.length <= files.length) {
      throw new Error("gopls produced no document symbols");
    }
    const bundle = {
      analyzer: {
        analyzerId: "gopls",
    ...
    console.error(
      `[aka-gopls] wrote nodes=${nodes.length} edges=${edges.length} chunks=${chunks.length} toolVersion=${toolVersion}`,
    );
  } finally {
    try {
