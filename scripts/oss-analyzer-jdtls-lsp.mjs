#!/usr/bin/env node
// Convert jdtls LSP document symbols into an aka-facts OSS analyzer bundle.
//
// This adapter is intentionally outside the AKA runtime. It starts the external
// open-source jdtls language server, asks it for document symbols, and
...
const DEFAULT_SERVER = "jdtls";
const DEFAULT_TIMEOUT_SECS = 300;
const DEFAULT_INITIALIZE_WAIT_SECS = 30;
const DEFAULT_REQUEST_TIMEOUT_SECS = 180;
...
function usage() {
  console.error(`Usage:
  scripts/oss-analyzer-jdtls-lsp.mjs --repo PATH --out PATH [options]
...
Options:
  --server CMD             jdtls LSP command. Default: ${DEFAULT_SERVER}
  ...
  --initialize-wait-secs N Wait after initialize for JDTLS project import. Default: ${DEFAULT_INITIALIZE_WAIT_SECS}
  --request-timeout-secs N Per-file documentSymbol timeout. Default: ${DEFAULT_REQUEST_TIMEOUT_SECS}
...
Example:
  scripts/oss-analyzer-jdtls-lsp.mjs \\
    --repo /src/cassandra \\
    --out /src/cassandra/.aka/jdtls-oss-analyzer-facts.json \\
...
function parseArgs(argv) {
  const args = {
    ...
    initializeWaitSecs: DEFAULT_INITIALIZE_WAIT_SECS,
    requestTimeoutSecs: DEFAULT_REQUEST_TIMEOUT_SECS,
  ...
  for (let i = 2; i < argv.length; i += 1) {
    ...
    switch (arg) {
      ...
      case "--timeout-secs":
        ...
        break;
      case "--initialize-wait-secs":
        args.initializeWaitSecs = Number(next());
        break;
      case "--request-timeout-secs":
        args.requestTimeoutSecs = Number(next());
        break;
      case "--max-files":
  ...
  if (!Number.isFinite(args.initializeWaitSecs) || args.initializeWaitSecs < 0) {
    throw new Error("--initialize-wait-secs must be zero or positive");
  }
  if (!Number.isFinite(args.requestTimeoutSecs) || args.requestTimeoutSecs <= 0) {
    throw new Error("--request-timeout-secs must be a positive number");
  }
  ...
  constructor(child, deadlineAt) {
    ...
    child.stderr.on("data", (chunk) => {
      for (const line of chunk.toString("utf8").split(/\r?\n/)) {
        if (line.trim()) {
          console.error(`[jdtls] ${line}`);
        }
      }
    });
    child.on("exit", (code, signal) => {
      const err = new Error(`jdtls language server exited code=${code} signal=${signal}`);
  ...
  handleServerRequest(message) {
    switch (message.method) {
      case "workspace/configuration": {
        ...
        this.respond(
          message.id,
          items.map((item) => javaConfigurationForSection(item?.section)),
        );
        break;
      }
...
function javaAnalyzerConfiguration() {
  return {
    java: {
      autobuild: {
        enabled: false,
...
function javaConfigurationForSection(section) {
  const config = javaAnalyzerConfiguration();
  switch (section) {
    case "java":
      return config.java;
    case "redhat":
      return config.redhat;
    case "redhat.telemetry":
      return config.redhat.telemetry;
    case "java.autobuild":
      return config.java.autobuild;
    case "java.configuration":
      return config.java.configuration;
    case "java.import":
      return config.java.import;
    case "java.project":
      return config.java.project;
    default:
      return config;
  }
}

async function discoverJavaFiles(root, maxFiles, excludeDirs) {
  const files = [];
  ...
  async function walk(dir) {
    ...
    for (const entry of entries) {
      ...
      } else if (entry.isFile() && JAVA_EXTENSIONS.has(path.extname(entry.name))) {
...
function flattenDocumentSymbols(symbols, fileNodeId, rel, lines, options) {
  ...
  function visit(symbol, parents) {
    ...
    if (parent?.nodeId) {
      edges.push({
        id: stableId("jdtls:edge:contains", `${parent.nodeId}->${nodeId}`),
        sourceId: parent.nodeId,
        targetId: nodeId,
        type: "CONTAINS",
        confidence: 1,
        reason: "jdtls documentSymbol hierarchy",
        evidence: { source: "lsp", rule: "documentSymbol", filePath: rel },
      });
    } else {
      edges.push({
        id: stableId("jdtls:edge:defines", `${fileNodeId}->${nodeId}`),
        sourceId: fileNodeId,
        targetId: nodeId,
        type: "DEFINES",
        confidence: 1,
        reason: "jdtls documentSymbol top-level definition",
        evidence: { source: "lsp", rule: "documentSymbol", filePath: rel },
      });
    }
    if (!options.noChunks) {
...
async function main() {
  ...
  const files = await discoverJavaFiles(repo, args.maxFiles, args.excludeDirs);
  if (files.length === 0) {
    throw new Error(`no Java files found in ${repo}`);
  }
  console.error(`[aka-jdtls] repo=${repo} files=${files.length} out=${out} server=${args.server}`);
  ...
  try {
    initResult = await rpc.request("initialize", {
      ...
      clientInfo: { name: "aka-jdtls-lsp-adapter", version: "0.1" },
      rootPath: repo,
      rootUri,
      ...
      initializationOptions: javaAnalyzerConfiguration(),
    });
    rpc.notify("initialized", {});
    rpc.notify("workspace/didChangeConfiguration", {
      settings: javaAnalyzerConfiguration(),
    });

    const toolVersion = args.toolVersion || initResult?.serverInfo?.version || "unknown";
    if (args.initializeWaitSecs > 0) {
      const waitMs = Math.min(args.initializeWaitSecs * 1000, rpc.remainingMs());
      console.error(`[aka-jdtls] waiting ${Math.round(waitMs / 1000)}s for JDTLS workspace import`);
      await sleep(waitMs);
    }
    ...
    await mapWithConcurrency(files, args.concurrency, async (file, index) => {
      ...
      if (index % args.logEvery === 0) {
        console.error(
          `[aka-jdtls] documentSymbol ${index + 1}/${files.length} ${rel} concurrency=${args.concurrency}`,
        );
      }
      const text = await readFile(file, "utf8");
      const uri = pathToFileURL(file).toString();
      const fileNodeId = stableId("jdtls:file", rel);
      const fileNodes = [{
        id: fileNodeId,
        label: "File",
        properties: {
      ...
      try {
        const symbols = await rpc.request(
          ...
          { textDocument: { uri } },
          Math.min(args.requestTimeoutSecs * 1000, rpc.remainingMs()),
        );
        ...
        console.error(`[aka-jdtls] skip ${rel}: ${error.message}`);
      } finally {
    ...
    if (nodes.length <= files.length) {
      throw new Error("jdtls produced no document symbols");
    }
    const bundle = {
      analyzer: {
        analyzerId: "jdtls",
    ...
    console.error(
      `[aka-jdtls] wrote nodes=${nodes.length} edges=${edges.length} chunks=${chunks.length} toolVersion=${toolVersion}`,
    );
  } finally {
    try {
