#!/usr/bin/env node
// Convert rust-analyzer LSP document symbols into an aka-facts OSS analyzer bundle.
//
// This adapter is intentionally outside the AKA runtime. It starts the external
// open-source rust-analyzer language server, asks it for document symbols, and
...
const DEFAULT_SERVER = "rust-analyzer";
...
function usage() {
  console.error(`Usage:
  scripts/oss-analyzer-rust-analyzer-lsp.mjs --repo PATH --out PATH [options]
...
Options:
  --server CMD             rust-analyzer LSP command. Default: ${DEFAULT_SERVER}
...
Example:
  scripts/oss-analyzer-rust-analyzer-lsp.mjs \\
    --repo /src/rust \\
    --out /src/rust/.aka/rust-analyzer-oss-analyzer-facts.json \\
...
class JsonRpc {
  constructor(child, deadlineAt) {
    ...
    child.on("exit", (code, signal) => {
      const err = new Error(`rust-analyzer language server exited code=${code} signal=${signal}`);
  ...
  handleServerRequest(message) {
    switch (message.method) {
      case "workspace/configuration": {
        ...
        this.respond(
          message.id,
          items.map(() => rustAnalyzerConfiguration()),
        );
        break;
      }
...
function rustAnalyzerConfiguration() {
  return {
    cargo: {
      allTargets: false,
      buildScripts: {
    ...
    files: {
      excludeDirs: ["target", ".git", "node_modules", "vendor"],
    },
    procMacro: {
      enable: false,
    },
...
async function discoverRustFiles(root, maxFiles, excludeDirs) {
  const files = [];
  ...
  async function walk(dir) {
    ...
    for (const entry of entries) {
      ...
      } else if (entry.isFile() && RUST_EXTENSIONS.has(path.extname(entry.name))) {
...
function flattenDocumentSymbols(symbols, fileNodeId, rel, lines, options) {
  ...
  function visit(symbol, parents) {
    ...
    if (parent?.nodeId) {
      edges.push({
        id: stableId("rust-analyzer:edge:contains", `${parent.nodeId}->${nodeId}`),
        sourceId: parent.nodeId,
        targetId: nodeId,
        type: "CONTAINS",
        confidence: 1,
        reason: "rust-analyzer documentSymbol hierarchy",
        evidence: { source: "lsp", rule: "documentSymbol", filePath: rel },
      });
    } else {
      edges.push({
        id: stableId("rust-analyzer:edge:defines", `${fileNodeId}->${nodeId}`),
        sourceId: fileNodeId,
        targetId: nodeId,
        type: "DEFINES",
        confidence: 1,
        reason: "rust-analyzer documentSymbol top-level definition",
        evidence: { source: "lsp", rule: "documentSymbol", filePath: rel },
      });
    }
    if (!options.noChunks) {
      ...
      if (text.trim()) {
        chunks.push({
          nodeId,
          kind: "rust-analyzer-document-symbol",
          filePath: rel,
...
async function main() {
  ...
  const files = await discoverRustFiles(repo, args.maxFiles, args.excludeDirs);
  if (files.length === 0) {
    throw new Error(`no Rust files found in ${repo}`);
  }
  console.error(`[aka-rust-analyzer] repo=${repo} files=${files.length} out=${out} server=${args.server}`);
  ...
  try {
    initResult = await rpc.request("initialize", {
      ...
      clientInfo: { name: "aka-rust-analyzer-lsp-adapter", version: "0.1" },
      rootPath: repo,
      rootUri,
      ...
      initializationOptions: rustAnalyzerConfiguration(),
    });
    ...
    await mapWithConcurrency(files, args.concurrency, async (file, index) => {
      ...
      if (index % args.logEvery === 0) {
        console.error(
          `[aka-rust-analyzer] documentSymbol ${index + 1}/${files.length} ${rel} concurrency=${args.concurrency}`,
        );
      }
      const text = await readFile(file, "utf8");
      const uri = pathToFileURL(file).toString();
      const fileNodeId = stableId("rust-analyzer:file", rel);
      const fileNodes = [{
        id: fileNodeId,
        label: "File",
        properties: {
      ...
      try {
        ...
        console.error(`[aka-rust-analyzer] skip ${rel}: ${error.message}`);
      } finally {
    ...
    if (nodes.length <= files.length) {
      throw new Error("rust-analyzer produced no document symbols");
    }
    const bundle = {
      analyzer: {
        analyzerId: "rust-analyzer",
        toolVersion,
      },
      stats: {
        files: files.length,
    ...
    console.error(
      `[aka-rust-analyzer] wrote nodes=${nodes.length} edges=${edges.length} chunks=${chunks.length} toolVersion=${toolVersion}`,
    );
  } finally {
    try {
