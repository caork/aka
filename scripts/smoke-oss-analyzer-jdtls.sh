#!/usr/bin/env bash
# Smoke optional jdtls LSP enrichment on a real Java repository.
# AKA runtime imports the generated aka-facts bundle; it does not start jdtls.
set -euo pipefail

usage() {
  cat <<'EOF'
Usage:
  scripts/smoke-oss-analyzer-jdtls.sh --repo /path/to/repo [options]
...
Options:
  --facts PATH             Existing/generated aka-facts bundle path.
                           Defaults to REPO/.aka/jdtls-oss-analyzer-facts.json.
  --server CMD             jdtls LSP command. Default: jdtls
  ...
  --min-lines N            Required Java source line count. Default: 500000.
  --aka-home PATH          Isolated AKA_HOME. Defaults to a temp dir.
  --index-max-secs N       Global indexing budget. Default: 1800.
  --enrichment-max-secs N  Optional jdtls import budget. Default: 600.
  --adapter-timeout-secs N jdtls adapter generation budget. Default: 1200.
  --initialize-wait-secs N Wait after initialize for JDTLS project import. Default: 60.
  --request-timeout-secs N Per-file documentSymbol timeout. Default: 180.
  ...
  --exclude-dir PATH       Additional repo-relative directory to skip. Repeatable.
                           Defaults skip framework-docs, test/simulator, and
                           test/unit, and tools/stress.
  --concurrency N          Concurrent documentSymbol requests. Default: 4.
  --allow-small-repo       Allow Java source lines below --min-lines for adapter debugging.
  --keep-aka-home          Do not delete the temporary AKA_HOME.
  --skip-generate          Reuse --facts without running jdtls.
  --dry-run                Validate inputs and generated settings, then exit.

Example:
  scripts/smoke-oss-analyzer-jdtls.sh --repo ~/src/cassandra \
    --server 'jdtls -data /tmp/aka-jdtls-workspace' \
    --tool-version jdtls-2026-06-26 \
    --query Cassandra --context StorageService
EOF
}
...
initialize_wait_secs=60
request_timeout_secs=180
max_files=""
exclude_dirs=("framework-docs" "test/simulator" "test/unit" "tools/stress")
concurrency=4
allow_small_repo=0
keep_aka_home=0
skip_generate=0
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --adapter-timeout-secs)
      ...
      shift 2
      ;;
    --initialize-wait-secs)
      initialize_wait_secs="${2:-}"
      shift 2
      ;;
...
repo="$(cd "${repo}" && pwd -P)"
facts_path="${facts_path:-${repo}/.aka/jdtls-oss-analyzer-facts.json}"
...
echo "==> repo: ${repo}"
echo "==> Java source lines: ${line_count}"
echo "==> facts bundle: ${facts_path}"
echo "==> jdtls server: ${server}"

if [[ "${line_count}" -lt "${min_lines}" ]]; then
  if [[ "${allow_small_repo}" -eq 0 ]]; then
    die "Java source line count ${line_count} is below requested ${min_lines}; pass --allow-small-repo only for adapter debugging"
  fi
  echo "warning: Java source line count ${line_count} is below requested ${min_lines}" >&2
fi

if [[ -z "${aka_home}" ]]; then
  aka_home="$(mktemp -d "${TMPDIR:-/tmp}/aka-jdtls-smoke.XXXXXX")"
...
if [[ "${skip_generate}" -eq 0 ]]; then
  echo "==> generating aka-facts with external jdtls LSP"
  adapter_args=(
    --repo "${repo}"
    --out "${facts_path}"
    --server "${server}"
    --timeout-secs "${adapter_timeout_secs}"
    --initialize-wait-secs "${initialize_wait_secs}"
  ...
  node "${repo_root}/scripts/oss-analyzer-jdtls-lsp.mjs" "${adapter_args[@]}"
fi
...
log_path="${aka_home}/jdtls-smoke.log"
echo "==> running baseline indexing plus optional jdtls enrichment"
(
  cd "${repo_root}"
...
grep -Eq 'provider=aka-facts-file merged|optional enrichment outcome' "${log_path}" \
  || die "jdtls facts enrichment outcome was not visible; see ${log_path}"

if grep -Eq 'provider=aka-facts-file failed|merge_failed|invalid_provenance' "${log_path}"; then
  die "jdtls facts enrichment failed; see ${log_path}"
fi
...
echo "==> jdtls OSS analyzer smoke passed"
